...
while true
   ...
   if 1 < i && (map(i-1, j) == 1 || map(i-1, j) == 6) && g(i-1, j) > g(i, j) + 1
       map(i-1, j) = 4;
   ...
   if i < nrows && (map(i+1, j) == 1 || map(i+1, j) == 6) && g(i+1, j) > g(i, j) + 1
       map(i+1, j) = 4;
       parent(i+1, j) = current;
       g(i+1, j) = g(i, j) + 1; 
       f(i+1, j) = g(i+1, j) + H(i+1, j);
   ...
   if 1 < j && (map(i, j-1) == 1 || map(i, j-1) == 6) && g(i, j-1) > g(i, j) + 1
       map(i, j-1) = 4;
   ...
   if j < ncols && (map(i, j+1) == 1 || map(i, j+1) == 6) && g(i, j+1) > g(i, j) + 1
       map(i, j+1) = 4;
       parent(i, j+1) = current;
       g(i, j+1) = g(i, j) + 1; 
       f(i, j+1) = g(i, j+1) + H(i, j+1);
