...
while true
   ...
   if 1 < i && (map(i-1, j) == 1 || map(i-1, j) == 6)
       map(i-1, j) = 4;
       parent(i-1, j) = current;
       distanceFromStart(i-1, j) = distanceFromStart(current) + 1;
   end
   ...
   if 1 < j && (map(i, j-1) == 1 || map(i, j-1) == 6)
       map(i, j-1) = 4;
       parent(i, j-1) = current;
       distanceFromStart(i, j-1) = distanceFromStart(current) + 1;
   end
